...
function updateDates(){
  ...
  const remDays = Math.max(0, Math.ceil(diff/86400000));
  todayEl.textContent="Today: "+fmt(today);
  expiryEl.textContent="Expiry: "+exp.toLocaleString();
  remainEl.textContent="Remaining: "+remDays+" days";
}
...
if(cname){ document.getElementById("clientLine").textContent = "Client: " + cname; }
...
async function checkSession(){
  ...
  if(!j.ok){ localStorage.clear(); location.href="login.html"; }
}
...
stopBtn.onclick=()=>{
  beep(); stopped=true; line("⛔ STOPPED");
  setTimeout(()=>location.reload(),300);
};

clearBtn.onclick=()=>{
  beep(); log.textContent=""; sent=0; failed=0; updateStats();
};
